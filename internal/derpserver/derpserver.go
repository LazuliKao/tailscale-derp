...
//
// This local copy is pinned to tailscale.com/derp/derpserver v1.102.3;
// see UPSTREAM.txt for the exact source revision. The client-admission
// callback below is maintained by this repository.
package derpserver
...
// SetVerifyClientFunc sets an additional client-admission check.
//
// The callback is invoked for every normal (non-mesh) client after the
// built-in tailscaled and URL checks have succeeded. A non-nil callback is
// therefore composed with those checks using logical AND. Mesh peers bypass
// all admission checks. The callback receives a context bounded to five
// seconds, the client's node key, and its source IP. The callback must be set
// before serving begins.
func (s *Server) SetVerifyClientFunc(f func(context.Context, key.NodePublic, netip.Addr) error) {
...
func (s *Server) verifyClient(ctx context.Context, clientKey key.NodePublic, info *derp.ClientInfo, clientIP netip.Addr) error {
	...
	// Local callback verification is an additional check after the built-in
	// checks above. It is intentionally skipped for trusted mesh peers above.
	if verify := s.verifyClientFunc; verify != nil {
		callbackCtx, cancel := context.WithTimeout(ctx, 5*time.Second)
		defer cancel()
		if err := verify(callbackCtx, clientKey, clientIP); err != nil {
			return fmt.Errorf("client-admission callback: %w", err)
		}
	}
	return nil
}
